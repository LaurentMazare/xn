#![cfg(feature = "metal")]
//! Metal backend tests. Most tests run an op on both the CPU backend and the
//! Metal backend from identical inputs and assert the outputs match, so the
//! CPU backend acts as the reference oracle. A few tests also check explicit
//! expected values.

use std::cell::RefCell;
use xn::{CPU, Result, Tensor, metal_backend::Device as Mt};
...
thread_local! {
    static DEVICE: RefCell<Option<Mt>> = const { RefCell::new(None) };
}

fn dev() -> Mt {
    DEVICE.with(|d| {
        d.borrow_mut()
            .get_or_insert_with(|| Mt::new(0).expect("failed to init metal device"))
            .clone()
    })
}
...
fn assert_close(a: &[f32], b: &[f32], tol: f32) {
    ...
    for (i, (x, y)) in a.iter().zip(b.iter()).enumerate() {
        ...
        assert!(d <= tol + tol * scale, "mismatch at {i}: cpu={x} mt={y} (|d|={d})");
    }
}
...
fn roundtrip_f16_bf16() -> Result<()> {
    ...
    let t: Tensor<half::f16, Mt> = Tensor::from_vec(d16.clone(), vec![3], &dev())?;
    assert_eq!(t.to_vec()?, d16);
    let db: Vec<half::bf16> = [1.0f32, 2.0, 3.0].into_iter().map(half::bf16::from_f32).collect();
    let t: Tensor<half::bf16, Mt> = Tensor::from_vec(db.clone(), vec![3], &dev())?;
...
fn zeros_and_full() -> Result<()> {
    let z: Tensor<f32, Mt> = Tensor::zeros(vec![3, 4], &dev())?;
    assert!(z.to_vec()?.iter().all(|&x| x == 0.0));
    let f: Tensor<f32, Mt> = Tensor::full(42.0, vec![2, 3], &dev())?;
...
fn dtype_roundtrip_conversions() -> Result<()> {
    // f32 -> f16 -> f32 on Metal should match doing it via CPU.
    let data = iota(20);
    let mt: Tensor<f32, Mt> = Tensor::from_vec(data.clone(), vec![20], &dev())?;
    let cpu: Tensor<f32, _> = Tensor::from_vec(data, vec![20], &CPU)?;
    let mt = mt.to::<half::f16>()?.to::<f32>()?;
    let cpu = cpu.to::<half::f16>()?.to::<f32>()?;
    assert_close(&cpu.to_vec()?, &mt.to_vec()?, 1e-6);
    Ok(())
}
...
macro_rules! cmp_binary {
    ($name:ident, $method:ident) => {
        ...
        fn $name() -> Result<()> {
            ...
            let av: Tensor<f32, Mt> = Tensor::from_vec(a.clone(), vec![8, 8], &dev())?;
            let bv: Tensor<f32, Mt> = Tensor::from_vec(b.clone(), vec![8, 8], &dev())?;
...
macro_rules! cmp_unary {
    ($name:ident, $method:ident, $tol:expr) => {
        ...
        fn $name() -> Result<()> {
            ...
            let av: Tensor<f32, Mt> = Tensor::from_vec(a.clone(), vec![64], &dev())?;
...
fn scale_affine() -> Result<()> {
    let a = iota(32);
    let av: Tensor<f32, Mt> = Tensor::from_vec(a.clone(), vec![32], &dev())?;
...
fn cmp_matmul(m: usize, k: usize, n: usize, batch: usize) -> Result<()> {
    ...
    let av: Tensor<f32, Mt> = Tensor::from_vec(a.clone(), as_.clone(), &dev())?;
    let bv: Tensor<f32, Mt> = Tensor::from_vec(b.clone(), bs.clone(), &dev())?;
...
fn matmul_t_and_transposed_view() -> Result<()> {
    ...
    let a = iota(6 * 4);
    let b = iota(5 * 4);
    let av: Tensor<f32, Mt> = Tensor::from_vec(a.clone(), vec![6, 4], &dev())?;
    let bv: Tensor<f32, Mt> = Tensor::from_vec(b.clone(), vec![5, 4], &dev())?;
...
fn cat_and_narrow() -> Result<()> {
    ...
    let av: Tensor<f32, Mt> = Tensor::from_vec(a.clone(), vec![2, 3, 4], &dev())?;
    let bv: Tensor<f32, Mt> = Tensor::from_vec(b.clone(), vec![2, 2, 4], &dev())?;
...
fn softmax_cmp() -> Result<()> {
    let data = iota(6 * 10);
    let mt: Tensor<f32, Mt> = Tensor::from_vec(data.clone(), vec![6, 10], &dev())?;
    let cpu: Tensor<f32, _> = Tensor::from_vec(data, vec![6, 10], &CPU)?;
    assert_close(&cpu.softmax()?.to_vec()?, &mt.softmax()?.to_vec()?, 1e-6);
    Ok(())
}
...
fn rms_norm_cmp() -> Result<()> {
    ...
    let mt: Tensor<f32, Mt> = Tensor::from_vec(data.clone(), vec![4, 16], &dev())?;
    let wv: Tensor<f32, Mt> = Tensor::from_vec(w.clone(), vec![16], &dev())?;
    let cpu: Tensor<f32, _> = Tensor::from_vec(data, vec![4, 16], &CPU)?;
    let wc: Tensor<f32, _> = Tensor::from_vec(w, vec![16], &CPU)?;
    assert_close(&cpu.rms_norm(&wc, 1e-5)?.to_vec()?, &mt.rms_norm(&wv, 1e-5)?.to_vec()?, 1e-5);
    Ok(())
}
...
fn layer_norm_cmp() -> Result<()> {
    ...
    let mt: Tensor<f32, Mt> = Tensor::from_vec(data.clone(), vec![4, 16], &dev())?;
    let wv: Tensor<f32, Mt> = Tensor::from_vec(w.clone(), vec![16], &dev())?;
    let bv: Tensor<f32, Mt> = Tensor::from_vec(bias.clone(), vec![16], &dev())?;
    let cpu: Tensor<f32, _> = Tensor::from_vec(data, vec![4, 16], &CPU)?;
    let wc: Tensor<f32, _> = Tensor::from_vec(w, vec![16], &CPU)?;
    let bc: Tensor<f32, _> = Tensor::from_vec(bias, vec![16], &CPU)?;
    assert_close(
        &cpu.layer_norm(&wc, &bc, 1e-5)?.to_vec()?,
        &mt.layer_norm(&wv, &bv, 1e-5)?.to_vec()?,
        1e-5,
    );
    Ok(())
}
...
fn reductions_cmp() -> Result<()> {
    ...
    for dims in [vec![2usize, 3, 4], vec![24]] {
        let n: usize = dims.iter().product();
        let d = data[..n].to_vec();
        let mt: Tensor<f32, Mt> = Tensor::from_vec(d.clone(), dims.clone(), &dev())?;
        let cpu: Tensor<f32, _> = Tensor::from_vec(d, dims.clone(), &CPU)?;
        for dim in 0..dims.len() {
            assert_close(&cpu.max(dim)?.to_vec()?, &mt.max(dim)?.to_vec()?, 1e-6);
            assert_close(&cpu.min(dim)?.to_vec()?, &mt.min(dim)?.to_vec()?, 1e-6);
            assert_close(
                &cpu.sum_keepdim(vec![dim])?.to_vec()?,
                &mt.sum_keepdim(vec![dim])?.to_vec()?,
                1e-5,
            );
            assert_eq!(cpu.argmax(dim)?.to_vec()?, mt.argmax(dim)?.to_vec()?);
            assert_eq!(cpu.argmin(dim)?.to_vec()?, mt.argmin(dim)?.to_vec()?);
        }
    }
    Ok(())
}
...
fn broadcast_ops() -> Result<()> {
    ...
    let av: Tensor<f32, Mt> = Tensor::from_vec(a.clone(), vec![2, 3], &dev())?;
    let rv: Tensor<f32, Mt> = Tensor::from_vec(row.clone(), vec![1, 3], &dev())?;
    ...
    let cv: Tensor<f32, Mt> = Tensor::from_vec(col.clone(), vec![2, 1], &dev())?;
...
fn forced_midbatch_flush_with_scratch() -> Result<()> {
    ...
    let mut xv: Tensor<f32, Mt> = Tensor::from_vec(base.clone(), vec![32, 32], &d)?;
    let r32v: Tensor<f32, Mt> = Tensor::from_vec(row32.clone(), vec![1, 32], &d)?;
    let r64v: Tensor<f32, Mt> = Tensor::from_vec(row64.clone(), vec![1, 64], &d)?;
...
fn index_select_cmp() -> Result<()> {
    ...
    let dv: Tensor<f32, Mt> = Tensor::from_vec(data.clone(), vec![5, 3], &dev())?;
    let iv: Tensor<i64, Mt> = Tensor::from_vec(ids.clone(), vec![5], &dev())?;
...
fn rope_cmp() -> Result<()> {
    ...
    let xv: Tensor<f32, Mt> = Tensor::from_vec(x.clone(), vec![b, h, t, d], &dev())?;
    let cv: Tensor<f32, Mt> = Tensor::from_vec(cos.clone(), vec![max_pos, d / 2], &dev())?;
    let sv: Tensor<f32, Mt> = Tensor::from_vec(sin.clone(), vec![max_pos, d / 2], &dev())?;
...
fn conv_transpose1d_cmp() -> Result<()> {
    for (b, ic, oc, len, ks, stride) in [(1, 1, 1, 3, 3, 1), (1, 2, 3, 4, 3, 2), (2, 2, 2, 5, 2, 2)]
    {
        ...
        let sv: Tensor<f32, Mt> = Tensor::from_vec(src.clone(), vec![b, ic, len], &dev())?;
        let kv: Tensor<f32, Mt> = Tensor::from_vec(kern.clone(), vec![ic, oc, ks], &dev())?;
...
// since inputs/outputs are f16-rounded; kernels accumulate in f32).
// -----------------------------------------------------------------------------
...
fn f16_matmul_gemm_and_gemv() -> Result<()> {
    ...
    for (m, k, n) in [(1usize, 512usize, 384usize), (16, 128, 64)] {
        let a = iota(m * k);
        let b = iota(k * n);
        let av: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&a), vec![m, k], &dev())?;
        let bv: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&b), vec![k, n], &dev())?;
...
fn f16_elementwise_and_norm() -> Result<()> {
    let a: Vec<f32> = (0..64).map(|i| (i as f32) * 0.05 + 0.1).collect();
    let av: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&a), vec![64], &dev())?;
    ...
    let bv: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&b), vec![64], &dev())?;
    ...
    let data = iota(4 * 16);
    let w: Vec<f32> = (0..16).map(|i| 0.5 + i as f32 * 0.03).collect();
    let dv: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&data), vec![4, 16], &dev())?;
    let wv: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&w), vec![16], &dev())?;
...
fn f16_index_select_and_rope() -> Result<()> {
    ...
    let dv: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&data), vec![5, 3], &dev())?;
    let iv: Tensor<i64, Mt> = Tensor::from_vec(ids.clone(), vec![4], &dev())?;
    let dc: Tensor<f32, _> = Tensor::from_vec(data, vec![5, 3], &CPU)?;
    ...
    let xv: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&x), vec![b, h, t, d], &dev())?;
    let cv: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&cos), vec![mp, d / 2], &dev())?;
    let sv: Tensor<half::f16, Mt> = Tensor::from_vec(to_f16_vec(&sin), vec![mp, d / 2], &dev())?;
    let xc: Tensor<f32, _> = Tensor::from_vec(x, vec![b, h, t, d], &CPU)?;
...
// -----------------------------------------------------------------------------
// bf16 compute path (ushort storage, f32 compute; validated against the f32
...
fn bf16_gpu_roundtrip_exact() -> Result<()> {
    // a + 0 forces a GPU LOAD/STORE round trip through the bf16 kernel path;
    ...
    let n = data.len();
    let a: Tensor<half::bf16, Mt> = Tensor::from_vec(data.clone(), vec![n], &dev())?;
    let z: Tensor<half::bf16, Mt> = Tensor::zeros(vec![n], &dev())?;
...
fn bf16_matmul_gemm_and_gemv() -> Result<()> {
    for (m, k, n) in [(1usize, 512usize, 384usize), (16, 128, 64)] {
        let a = iota(m * k);
        let b = iota(k * n);
        let av: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&a), vec![m, k], &dev())?;
        let bv: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&b), vec![k, n], &dev())?;
        let ac: Tensor<f32, _> = Tensor::from_vec(a, vec![m, k], &CPU)?;
        let bc: Tensor<f32, _> = Tensor::from_vec(b, vec![k, n], &CPU)?;
...
fn bf16_elementwise_and_norm() -> Result<()> {
    let a: Vec<f32> = (0..64).map(|i| (i as f32) * 0.05 + 0.1).collect();
    let av: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&a), vec![64], &dev())?;
    let ac: Tensor<f32, _> = Tensor::from_vec(a.clone(), vec![64], &CPU)?;
    assert_close(&ac.silu()?.to_vec()?, &bf16_to_f32(&av.silu()?.to_vec()?), 5e-2);

    let b: Vec<f32> = a.iter().map(|x| x + 0.5).collect();
    let bv: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&b), vec![64], &dev())?;
    let bc: Tensor<f32, _> = Tensor::from_vec(b, vec![64], &CPU)?;
    assert_close(&ac.add(&bc)?.to_vec()?, &bf16_to_f32(&av.add(&bv)?.to_vec()?), 5e-2);

    let data = iota(4 * 16);
    let w: Vec<f32> = (0..16).map(|i| 0.5 + i as f32 * 0.03).collect();
    let dv: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&data), vec![4, 16], &dev())?;
    let wv: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&w), vec![16], &dev())?;
    let dc: Tensor<f32, _> = Tensor::from_vec(data.clone(), vec![4, 16], &CPU)?;
    let wc: Tensor<f32, _> = Tensor::from_vec(w, vec![16], &CPU)?;
    assert_close(
        &dc.rms_norm(&wc, 1e-5)?.to_vec()?,
...
fn bf16_index_select_and_rope() -> Result<()> {
    let data = iota(5 * 3);
    let ids = vec![0i64, 2, 4, 1, -1];
    let dv: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&data), vec![5, 3], &dev())?;
    let iv: Tensor<i64, Mt> = Tensor::from_vec(ids.clone(), vec![5], &dev())?;
    let dc: Tensor<f32, _> = Tensor::from_vec(data, vec![5, 3], &CPU)?;
    let ic: Tensor<i64, _> = Tensor::from_vec(ids, vec![5], &CPU)?;
    assert_close(
    ...
    let cos: Vec<f32> = (0..mp * d / 2).map(|i| (i as f32 * 0.3).cos()).collect();
    let sin: Vec<f32> = (0..mp * d / 2).map(|i| (i as f32 * 0.3).sin()).collect();
    let xv: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&x), vec![b, h, t, d], &dev())?;
    let cv: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&cos), vec![mp, d / 2], &dev())?;
    let sv: Tensor<half::bf16, Mt> = Tensor::from_vec(to_bf16_vec(&sin), vec![mp, d / 2], &dev())?;
    let xc: Tensor<f32, _> = Tensor::from_vec(x, vec![b, h, t, d], &CPU)?;
    let cc: Tensor<f32, _> = Tensor::from_vec(cos, vec![mp, d / 2], &CPU)?;
    let sc: Tensor<f32, _> = Tensor::from_vec(sin, vec![mp, d / 2], &CPU)?;
    assert_close(
