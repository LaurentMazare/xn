//! Basic Vulkan example demonstrating tensor operations on the GPU.
//!
//! Run with: cargo run --release --no-default-features --features vulkan --example basic_vulkan
//!
//! Requires a Vulkan driver (ICD). On an AMD APU under Linux install Mesa's
//! RADV driver (Arch: `vulkan-radeon`). Set `XN_VULKAN_DEVICE=<index>` to pick a
//! specific physical device.

use xn::{Backend, Result, Tensor, vulkan_backend::Device};

fn main() -> Result<()> {
    let device = Device::new(0)?;
    println!("Vulkan device initialized: {}", device.name());
