//! Basic WebGPU example demonstrating tensor operations on the GPU.
//!
//! Run with: cargo run --release --no-default-features --features webgpu --example basic_webgpu
//!
//! Uses the `wgpu` crate, which drives whatever GPU API is available (Vulkan,
//! Metal, DX12 or GL). Set `XN_WEBGPU_DEVICE=<index>` to pick a specific
//! adapter.
...
fn main() -> Result<()> {
    ...
    println!("WebGPU device initialized: {}", device.name());
