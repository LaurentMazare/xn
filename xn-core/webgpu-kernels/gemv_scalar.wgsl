// GEMV without the vec4 fast path of gemv.wgsl. That fast path binds `rhs` a
// second time as a vec4 view of the same buffer, and browser WebGPU validation
// rejects two storage bindings aliasing one buffer in a dispatch — so wasm
// builds use this scalar-only variant instead.
//   dst[b, 0, j] = sum_l lhs[b, 0, l] * rhs[b, l, j]
// One workgroup per output column j (per batch b); the workgroup's threads
// cooperatively reduce over the k dimension in f32. Params match the GEMM
// kernel. Grid: (n, batch, 1).
struct Params {
